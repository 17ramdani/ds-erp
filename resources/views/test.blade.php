                    ...
                    <form class="uk-form-stacked uk-position-relative">
                        ...
                        <div uk-grid>
                            <div class="uk-width-1-2@l">
                                ...
                                <div class="uk-margin-large-bottom">
                                    ...
                                    <div class="uk-form-controls">
                                        <select id="tipe_kain" name="tipe_kain" class="uk-select" required>
                                            <option>--Pilih--</option>
                                            @foreach ($tipeKain as $item)
                                                <tr onclick="changeGambar('{{ $item->nama }}')">
                                                    <td>{{ $item->nama }}</td>
                                                </tr>
                                            @endforeach
                                        </select>
                                ...
                                {{-- <div class="uk-margin-large-bottom">
                                    ...
                                    <div class="uk-form-controls">
                                        <input class="uk-input uk-form-width-medium" value="{{ date('Y-m-d') }}" name="target_kirim" type="date" placeholder="Input" aria-label="Input">
                            </div>
                        </div> --}}
                            </div>
                        ...
                        <div class="uk-margin" id="barang-list" hidden>
                            <div id="kain1" class="uk-margin">
                                ...
                                <div class="uk-margin-large-top">
                                    <div class="uk-form-controls">
                                        <div class="uk-margin">
                                            <input class="uk-input uk-form-width-large" type="text" id="search"
                                                placeholder="Search By warna..">
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="uk-margin-medium-top uk-child-width-1-2@s" uk-grid>
                            ...
                            <div class="uk-text-right@s">
                                <a href=""
                                    class="uk-button uk-button-default uk-border-rounded uk-disabled">Pesanan
                                    Development</a>
                            </div>
                        </div>
                    </form>
            ...
            $('#search').on('keyup', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // console.log(response.warnas)
                        // $.each(response.warnas, function(i3, item3) {
                        //     optwarnav += `<option value="${item3.warna.id}">${item3.warna.nama}</option>`;
                        // });

                        $.each(response.satuan, function(i2, item2) {
                            optvalue +=
                                `<option value="${item2.keterangan}">${item2.keterangan}</option>`;
                        });
                        ...
                        // $('#tipe_acessories').html(optwarnav);
                        // document.getElementById("nama_atd").innerHTML= response.acesoris[0].nama;
                        // document.getElementById("lgm_atd").innerHTML= response.acesoris[0].lebar.keterangan+'/'+response.acesoris[0].gramasi.nama;
                        // $('#id_assesoris').val(response.acesoris[0].id);
                        // $('#nama_assesoris').val(response.acesoris[0].nama);
                    }
                });
            })
        </script>
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.count_keranjang < 1) {
                            $('#submit-message').text(response.message)
                            let link =
                                `<a href="pesanan" class="uk-button uk-button-primary">OK</a>`;
                            link = `<a class="uk-button uk-button-primary uk-modal-close">OK</a>`;
                            $('#link-modalSubmitOrder').html(link)
                            UIkit.modal('#modalSubmitOrder').show();
