                ...
                <div class="rz-dashboard">
                    ...
                    <div class="uk-margin-medium">
                        <form class="uk-child-width-1-5@m uk-child-width-1-2@s uk-form-stacked uk-grid-small" uk-grid
                            ...
                            <div>
                                <select class="uk-select" name="accessories_id">
                                    <option value="">Pilih Kategori Aksesoris</option>
                                    @foreach ($acs as $cc)
                                    <option value="{{ $cc->id }}"
                                        {{ $cc->id == request('accessories_id') ? 'selected' : '' }}>
                                        {{ $cc->name }}
                                    </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="rz-panel">
                        ...
                        <div class="uk-flex uk-flex-between">
                            <div>
                                <h4>List Tipe Kain Aksesoris</h4>
                            </div>
                        </div>
                                            ...
                                            <form action="{{ route('tipe-price.store') }}" method="POST">
                                                ...
                                                @foreach ($groupedTipePrices as $tipeKainId => $groupedTipePrice)
                                                    <tr>
                                                        ...
                                                        <td> {{ tipeKain->jenis->nama }}</td>
                                                        <td> {{ tipeKain->nama }}</td>
                                                        <td> {{ tipeKain->kategori_warna->nama }}</td>
                                                        <td> {{ acs->name }}</td>
                                                        <td> {{ paket }}</td>
                                                        <td> {{ status }}</td>
                                                    </tr>
